...
function getTouchedForFields(fields) {
  return fields.reduce((touched, field) => ({ ...touched, [field]: true }), {})
}
...
function ProductInfoEditForm({
  ...
  const validateStepBeforeLeave = async (formik, stepIndex) => {
    ...
    formik.setTouched({ ...formik.touched, ...getTouchedForFields(fields) }, true)
      ...
      <Formik
        ...
        onSubmit={async (values, actions) => {
          try {
            ...
            const payload = buildProductInfoPayload(formValues, mainImage, subImages, { mode: 'edit' })
...
function SingleVariantForm({ mode, initialValues, productValues, onSave, onCancel, isSaving }) {
      ...
      {(formik) => {
        ...
        return (
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_18px_50px_rgba(15,23,42,0.05)] sm:p-6">
            <p className="mb-1 text-sm font-bold text-slate-900">Variation type</p>
            <p className="mb-3 text-xs text-slate-500">Choose a common type or enter a custom one below.</p>
              ...
              {ATTRIBUTE_PRESETS.map((preset) => (
                <button
                  ...
                  className={`cursor-pointer rounded-lg border px-3 py-1.5 text-xs font-semibold transition-colors ${
                    formik.values.attribute === preset
            ...
            <ProductInput
              ...
              name="attribute"
              label="Custom type"
              hint="Or type your own (e.g. Voltage, Finish, Fragrance)"
          ...
          {/* Core fields */}
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_18px_50px_rgba(15,23,42,0.05)] sm:p-6">
            <p className="mb-4 text-xs font-bold uppercase tracking-widest text-slate-400">Variant details</p>
          ...
          {/* Pricing */}
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_18px_50px_rgba(15,23,42,0.05)] sm:p-6">
            <p className="mb-4 text-xs font-bold uppercase tracking-widest text-slate-400">Pricing</p>
            <div className="grid gap-4 sm:grid-cols-2">
              <ProductMoneyInput
                id="price"
                name="price"
          ...
          <div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            >
              {formik.isSubmitting || isSaving ? (
                ...
                <>Add variant <ArrowRight className="size-4" /></>
              )}
            </button>
          </div>
          </Form>
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between lg:gap-8">
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1.5 max-w-2xl text-sm leading-relaxed text-slate-600">
              Edit or remove existing variants one at a time. Add new variants to expand your product options.
            </p>
          </div>
...
function VariationsEditForm({ productId, formState, onFinished }) {
  ...
  if (mode === 'add') {
    return (
      <div className="page-enter space-y-5">
        {pageHeader('Add new variant', 'Define a new product option with its own price, stock, and images.')}
        <SingleVariantForm
          mode="add"
          initialValues={{ ...EMPTY_VARIANT_VALUES }}
          productValues={productValues}
