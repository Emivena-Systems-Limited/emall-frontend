...
export default function PromotionFiltersDrawer({
  open,
  onClose,
  statusFilter,
  ...
  const activeDatePreset = SCHEDULE_DATE_PRESETS.find((preset) =>
  ...
  const activeFilterCount = useMemo(
    () => countPromotionDrawerFilters({ statusFilter, dateRange }),
  ...
  return createPortal(
    <>
      ...
      <aside
        role="dialog"
        aria-modal="true"
        aria-labelledby="promotion-filters-title"
      ...
      >
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div className="min-w-0">
                <h2 id="promotion-filters-title" className="text-lg font-bold text-slate-900">
                  Filter promotions
                </h2>
                <p className="mt-0.5 text-sm text-slate-500">
                  {activeFilterCount > 0
                    ? `${activeFilterCount} filter${activeFilterCount === 1 ? '' : 's'} applied`
                    : 'Choose a status or schedule date range'}
                </p>
              </div>
            </div>
            <button
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4 sm:p-5">
          <FilterCard
            icon={Tag}
            title="Status"
            description="Filter by promotion lifecycle state"
          >
            <button
              type="button"
          ...
          <FilterCard
            icon={CalendarRange}
            title="Schedule"
            description="Promotions overlapping the selected period"
          >
            <FilterOptionGrid>
            ...
            {customDateOpen && (
              <div className="space-y-3 rounded-xl bg-slate-50 p-3 ring-1 ring-slate-100">
                ...
                {hasCustomDateRange(dateRange) && (
                  <button
                    type="button"
                    onClick={() => onDateRangeChange(DEFAULT_PROMOTION_DATE_RANGE)}
        ...
        <div className="shrink-0 space-y-2 border-t border-slate-200 bg-white p-4 sm:px-5">
          ...
          >
            Show {resultCount} promotion{resultCount === 1 ? '' : 's'}
          </button>
        </div>
      </aside>
    </>,
...
export function countPromotionDrawerFilters({
  statusFilter = STATUS_FILTERS.ALL,
  dateRange = DEFAULT_PROMOTION_DATE_RANGE,
} = {}) {
  return [
    statusFilter !== STATUS_FILTERS.ALL,
    dateRange.startDate,
