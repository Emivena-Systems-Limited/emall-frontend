...
function formatFulfillment(channel) {
  const token = String(channel ?? '').trim().toLowerCase()
  if (!token) return null
  if (token === 'vendor') return 'Vendor fulfilled'
  if (token === 'platform') return 'Platform fulfilled'
...
function OrderLineItemCard({ item, orderId }) {
  ...
  const fulfillmentLabel = formatFulfillment(item.fulfillmentChannel)
  ...
  return (
    <article className="rounded-xl border border-slate-200 bg-slate-50/40 p-4 transition-colors hover:border-slate-300 hover:bg-white lg:rounded-none lg:border-0 lg:border-b lg:border-slate-100 lg:bg-transparent lg:p-0 lg:py-4 lg:last:border-b-0 lg:hover:bg-slate-50/50">
      <div className="grid gap-4 lg:grid-cols-[minmax(0,2.2fr)_minmax(0,1fr)_minmax(5rem,6rem)_minmax(6.5rem,7.5rem)_minmax(6.5rem,7.5rem)] lg:items-center lg:gap-4">
        <div className="flex min-w-0 items-start gap-3.5">
          ...
          <div className="min-w-0 flex-1 space-y-1.5">
            {productHref ? (
              <Link
                to={productHref}
                className="group inline-flex max-w-full items-start gap-1.5 text-sm font-bold leading-snug text-slate-900 transition hover:text-brand"
              >
                <span className="min-w-0">{item.productName}</span>
                <ExternalLink className="mt-0.5 size-3.5 shrink-0 opacity-0 transition group-hover:opacity-100" aria-hidden />
              </Link>
            ) : (
              <p className="text-sm font-bold leading-snug text-slate-900">{item.productName}</p>
            )}
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {fulfillmentLabel ? (
                <span className="inline-flex items-center gap-1 rounded-full bg-brand-light/60 px-2.5 py-1 text-[11px] font-semibold text-brand ring-1 ring-brand-muted/60">
...
export default function OrderLineItems({ items, orderId }) {
  ...
  return (
    <div>
      <div className="mb-3 hidden rounded-xl bg-slate-50 px-4 py-2.5 lg:grid lg:grid-cols-[minmax(0,2.2fr)_minmax(0,1fr)_minmax(5rem,6rem)_minmax(6.5rem,7.5rem)_minmax(6.5rem,7.5rem)] lg:gap-4">
      ...
      <div className="space-y-3 lg:space-y-0">
        {items.map((item) => (
          <OrderLineItemCard key={item.id} item={item} orderId={orderId} />
        ))}
      </div>
