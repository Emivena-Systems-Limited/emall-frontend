...
import { useDeleteCouponMutation } from '../../hooks/useAdminCoupons'

export default function CouponRemoveModal({ open, coupon, onClose, onRemoved }) {
  if (!open || !coupon) return null
  return <CouponRemoveForm key={coupon.id} coupon={coupon} onClose={onClose} onRemoved={onRemoved} />
}

function CouponRemoveForm({ coupon, onClose, onRemoved }) {
  const mutation = useDeleteCouponMutation()
  ...
  const handleConfirm = async () => {
    if (busy) return
    try {
      await mutation.mutateAsync({ id: coupon.id, status: coupon.status })
      onRemoved?.()
      onClose()
    } catch {
  ...
  return (
    <VendorDialog open onClose={handleClose} labelledBy="coupon-remove-title" widthClass="max-w-md">
      <div className="relative overflow-hidden">
        ...
        <div className="px-5 pt-4 sm:px-6">
          <h2 id="coupon-remove-title" className="text-xl font-bold tracking-tight text-slate-950">
            Remove this coupon?
          </h2>
          <p id="coupon-remove-copy" className="mt-1.5 text-sm leading-relaxed text-slate-500">
            Shoppers will no longer be able to use this code. Past orders keep their original discount.
          </p>
        </div>
      </div>

      <div className="space-y-3 px-5 py-4 sm:px-6" aria-describedby="coupon-remove-copy">
