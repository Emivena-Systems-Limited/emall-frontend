...
import { useDeleteProductMutation } from '../../hooks/useAdminProducts'

export default function ProductRemoveModal({ open, product, onClose, onRemoved }) {
  if (!open || !product) return null
  return <ProductRemoveForm key={product.id} product={product} onClose={onClose} onRemoved={onRemoved} />
}

function ProductRemoveForm({ product, onClose, onRemoved }) {
  ...
  const handleConfirm = async () => {
    if (busy) return
    try {
      await mutation.mutateAsync({ id: product.id, approvalStatus: product.approvalStatus })
      onRemoved?.()
      onClose()
    } catch {
  ...
  return (
    <VendorDialog open onClose={handleClose} labelledBy="product-remove-title" widthClass="max-w-md">
      <div className="relative overflow-hidden">
        ...
        <div className="px-5 pt-4 sm:px-6">
          <h2 id="product-remove-title" className="text-xl font-bold tracking-tight text-slate-950">
            Remove this listing?
          </h2>
          <p id="product-remove-copy" className="mt-1.5 text-sm leading-relaxed text-slate-500">
            This takes it out of the catalogue. The vendor will no longer see it in their store.
          </p>
        </div>
      </div>

      <div className="space-y-3 px-5 py-4 sm:px-6" aria-describedby="product-remove-copy">
