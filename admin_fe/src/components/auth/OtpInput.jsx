...
export default function OtpInput({ value, onChange, error = false, disabled = false }) {
  ...
  const handlePaste = (event) => {
    ...
    onChange(pasted)
    focusInput(Math.min(Math.max(pasted.length, 1) - 1, OTP_LENGTH - 1))
  }
      ...
      <div className="flex justify-center gap-2 sm:gap-3">
        {digits.map((digit, index) => (
          <input
            ...
            className={`size-11 rounded-xl border-2 bg-white text-center text-lg font-semibold text-slate-900 outline-none transition-colors sm:size-12 sm:text-xl ${
              error
