...
import { useDeleteBrandMutation } from '../../hooks/useAdminBrands'

export default function BrandRemoveModal({ open, brand, onClose, onRemoved }) {
  if (!open || !brand) return null
  return <BrandRemoveForm key={brand.id} brand={brand} onClose={onClose} onRemoved={onRemoved} />
}

function BrandRemoveForm({ brand, onClose, onRemoved }) {
  const mutation = useDeleteBrandMutation()
  ...
  const handleConfirm = async () => {
    if (busy) return
    try {
      await mutation.mutateAsync({ id: brand.id, status: brand.status })
      onRemoved?.()
      onClose()
    } catch {
  ...
  return (
    <VendorDialog open onClose={handleClose} labelledBy="brand-remove-title" widthClass="max-w-md">
      <div className="relative overflow-hidden">
        ...
        <div className="px-5 pt-4 sm:px-6">
          <h2 id="brand-remove-title" className="text-xl font-bold tracking-tight text-slate-950">
            Remove this brand?
          </h2>
          <p id="brand-remove-copy" className="mt-1.5 text-sm leading-relaxed text-slate-500">
            This takes it out of the catalogue. Listings that used it will need another brand.
          </p>
        </div>
      </div>

      <div className="space-y-3 px-5 py-4 sm:px-6" aria-describedby="brand-remove-copy">
