...
function OrderLineItemCard({ item }) {
    ...
    <article className="rounded-xl border border-slate-200 bg-slate-50/40 p-4 transition-colors hover:border-slate-300 hover:bg-white lg:rounded-none lg:border-0 lg:border-b lg:border-slate-100 lg:bg-transparent lg:p-0 lg:py-4 lg:last:border-b-0 lg:hover:bg-slate-50/50">
      <div className="grid gap-4 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)_minmax(5rem,6rem)_minmax(6.5rem,7.5rem)_minmax(6.5rem,7.5rem)] lg:items-center lg:gap-4">
        <div className="flex min-w-0 items-start gap-3.5">
          {item.image ? (
            <span className="flex size-16 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-white ring-1 ring-slate-200">
              <img src={item.image} alt="" className="size-full object-contain p-1.5" />
            </span>
          ) : (
            <span className="flex size-16 shrink-0 items-center justify-center rounded-xl bg-white text-slate-400 ring-1 ring-slate-200">
          ...
          <div className="min-w-0 flex-1 space-y-1.5">
            {productHref ? (
              ...
              >
                ...
                <ExternalLink className="size-3.5 shrink-0 opacity-0 transition group-hover:opacity-100" aria-hidden="true" />
              </Link>
            ) : (
...
export default function OrderLineItems({ items }) {
  if (!items?.length) {
    return (
  ...
  return (
    <div>
      ...
      <div className="space-y-3 lg:space-y-0">
        {items.map((item) => (
          <OrderLineItemCard key={item.id} item={item} />
        ))}
      </div>
      ...
      <div className="mt-5 flex flex-col gap-3 border-t border-slate-100 pt-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="text-left sm:text-right">
          ...
          <p className="mt-0.5 text-lg font-bold tabular-nums text-slate-950">{formatOrderMoney(linesSubtotal)}</p>
        </div>
      </div>
    </div>
  )
