...
function Tip({ active, payload, total }) {
  ...
  return (
    <div className="rounded-xl border border-slate-100 bg-white/95 px-3.5 py-2.5 font-sans shadow-2xl backdrop-blur-sm">
      ...
      <p className="text-xs text-slate-500">{formatPercent(share)} of searches</p>
    </div>
  )
}

export default function SearchMixChart({
  mix = [],
  title = 'Where they searched',
  subtitle = 'Storefront versus app and other sources',
  ...
  const data = mix.map((slice) => ({
    ...slice,
    color: slice.accent ?? slice.color,
    ...
    <section className="flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_16px_45px_rgba(15,23,42,0.04)]">
      ...
      {data.length === 0 ? (
        ...
        <div className="flex flex-1 flex-col items-center gap-5">
          <div className="h-[220px] w-full max-w-[240px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <text x="50%" y="46%" textAnchor="middle" style={DONUT_CENTER_LABEL}>{centerLabel}</text>
          ...
          <ul className="w-full space-y-2.5" aria-label={title}>
            {data.map((slice) => (
              <li
                key={slice.key}
...
export function SearchMixSkeleton() {
  return (
    <section
