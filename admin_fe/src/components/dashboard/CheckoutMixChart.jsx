...
export default function CheckoutMixChart({ mix = [], total = 0 }) {
  const mixTotal = mix.reduce((sum, slice) => sum + (Number(slice.count) || 0), 0) || total
  ...
  return (
    <section className="flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_16px_45px_rgba(15,23,42,0.04)]">
      <div className="mb-4">
        <h2 className="text-sm font-bold text-slate-900">Baskets</h2>
        <p className="text-xs text-slate-500">Open carts versus those that reached pay</p>
      </div>
      ...
      <div className="flex flex-1 flex-col items-center gap-5">
        ...
        <ul className="w-full space-y-2.5" aria-label="Baskets still open versus reached checkout">
          {data.map((slice) => (
            <li
              key={slice.key}
