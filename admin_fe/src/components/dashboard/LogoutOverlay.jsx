...
export default function LogoutOverlay() {
  const mutatingCount = useIsMutating({ mutationKey: ['admin-auth', 'logout'] })
  ...
  useEffect(() => {
    if (isMutating || !visible) return undefined
  ...
  return (
    <div
      className="fade-in fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/45 px-4 backdrop-blur-sm"
      role="status"
      aria-live="polite"
      aria-label="Signing out"
    >
      <div className="w-full max-w-sm rounded-2xl border border-slate-200/80 bg-white p-8 text-center shadow-2xl shadow-slate-900/10">
        <div className="relative mx-auto mb-6 flex size-16 items-center justify-center">
          ...
          <span className="absolute -right-1 -bottom-1 flex size-7 items-center justify-center rounded-full bg-white ring-2 ring-white">
            <LogOut className="size-3.5 text-brand" strokeWidth={2.25} />
          </span>
        </div>
        <p className="text-base font-semibold tracking-tight text-slate-900">Signing you out</p>
        <p className="mt-1.5 text-sm text-slate-500">Securing this operator session…</p>
