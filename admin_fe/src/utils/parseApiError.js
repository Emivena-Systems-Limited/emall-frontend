...
export function assertAuthEnvelope(data, fallback = 'Request failed') {
  const envelope = unwrapApiEnvelope(data)
  if (!envelope || typeof envelope !== 'object') return envelope ?? data

  const reason = String(envelope.reason ?? '').toLowerCase()
  const failed = Boolean(envelope.in_error) || reason === 'failed' || reason === 'error'
  if (!failed) return envelope
