  ...
  return (
    ...
    >
      {[1, 2, 3, 4, 5].map((star) => (
        <Star
          key={star}
          className={`${size} ${star <= rating ? "fill-amber-400 text-amber-400" : "fill-slate-100 text-slate-200"}`}
        />
      ))}
    </span>
  );
    ...
    <section>
        ...
        <div className="flex min-w-max gap-2">
          {tabs.map((name) => (
            <button
              ...
              type="button"
              onClick={() => setTab(name)}
              className={`border-b-2 px-4 py-3 text-sm font-bold ${tab === name ? "border-auth-primary text-auth-primary" : "border-transparent text-slate-500 hover:text-slate-800"}`}
            >
              {name}
              <span
                className={`ml-2 rounded-full px-2 py-0.5 text-[0.65rem] ${tab === name ? "bg-red-50" : "bg-slate-100"}`}
              >
                {count(name)}
              </span>
            </button>
      ...
      <div className="mt-5 space-y-4">
            ...
            <div className="flex flex-col gap-5 xl:flex-row">
              ...
              <div className="min-w-0 flex-1 border-t border-slate-100 pt-4 xl:border-l xl:border-t-0 xl:pl-6 xl:pt-0">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <span
                    className={`rounded-full px-2.5 py-1 text-[0.68rem] font-bold ring-1 ring-inset ${statusTone[review.status]}`}
                  >
                    {review.status}
                  </span>
                </div>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      navigate(`/account/reviews/${review.id}/edit`)
                    }
        ...
        {!visible.length ? (
          <div className="flex min-h-72 flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center">
            ...
            <h3 className="mt-4 font-bold text-slate-900">No reviews found</h3>
      ...
      {viewing ? (
        <Dialog label="Review details" onClose={() => setViewing(null)}>
          <span
            className={`rounded-full px-3 py-1 text-xs font-bold ring-1 ring-inset ${statusTone[viewing.status]}`}
          >
            {viewing.status}
          </span>
      ...
      {deleting ? (
        <Dialog label="Delete review" onClose={() => setDeleting(null)}>
          ...
          <p className="mt-2 text-sm leading-6 text-slate-500">
            Your review for <strong>{deleting.product}</strong> will be
            permanently removed.
          </p>
...
function LeaveReview({ reviewId }) {
  ...
  const existing = reviewsSeed.find((review) => review.id === reviewId);
  ...
  const product =
    existing?.product ?? params.get("product") ?? "Wireless Head Set";
  ...
  const submit = (event) => {
    ...
    notify.success(
      editing ? "Review updated successfully" : "Review submitted successfully",
    );
        ...
        >
          <fieldset>
            ...
            >
              {[1, 2, 3, 4, 5].map((value) => (
                ...
                >
                  <Star
                    className={`size-8 ${value <= (hover || rating) ? "fill-amber-400 text-amber-400" : "fill-slate-100 text-slate-300"}`}
                  />
                </button>
              ))}
          ...
          <label className="block">
            ...
            <input
              ...
              className={`mt-2 h-12 w-full rounded-xl border px-4 text-sm outline-none focus:border-auth-primary ${errors.title ? "border-red-400" : "border-slate-200"}`}
            />
            {errors.title ? (
          ...
          <label className="block">
            ...
            <textarea
              ...
              className={`mt-2 w-full rounded-xl border p-4 text-sm leading-6 outline-none focus:border-auth-primary ${errors.body ? "border-red-400" : "border-slate-200"}`}
            />
          ...
          <fieldset>
            <legend className="text-sm font-bold text-slate-900">
              ...
              <span className="text-auth-primary">*</span>
            </legend>
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              {[
                ...
                <button
                  ...
                  className={`flex items-center gap-3 rounded-xl border p-4 text-left text-sm font-bold ${recommend === value ? "border-auth-primary bg-red-50 text-auth-primary" : "border-slate-200 text-slate-700"}`}
                >
                  <span
                    className={`flex size-5 items-center justify-center rounded-full border ${recommend === value ? "border-auth-primary bg-auth-primary text-white" : "border-slate-300"}`}
                  >
          ...
          <div className="flex flex-col-reverse gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:justify-end">
            ...
            <button
              type="submit"
              className="rounded-xl bg-auth-primary px-7 py-3 text-sm font-bold text-white hover:bg-auth-primary-hover"
            >
