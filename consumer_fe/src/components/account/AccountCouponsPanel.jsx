...
function CouponCard({ coupon, onCopy, onDetails }) {
  ...
  return (
    <article className={`group relative overflow-hidden rounded-3xl border bg-white shadow-[0_12px_35px_rgba(15,23,42,0.05)] transition hover:-translate-y-1 hover:shadow-[0_20px_45px_rgba(15,23,42,0.1)] ${inactive ? 'border-slate-200 opacity-75' : 'border-slate-200 hover:border-auth-primary/25'}`}>
      <div className={`bg-gradient-to-br ${coupon.tone} p-5 text-white`}>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="coupon-details-title">
      <div className="w-full max-w-lg overflow-hidden rounded-3xl bg-white shadow-2xl">
        <div className={`bg-gradient-to-br ${coupon.tone} p-6 text-white`}>
...
export default function AccountCouponsPanel() {
  ...
  return (
    <section>
      ...
      <div className="mt-6 overflow-hidden rounded-3xl bg-gradient-to-br from-[#a92f24] via-auth-primary to-[#e65b44] p-6 text-white shadow-[0_18px_45px_rgba(192,57,43,0.2)] sm:p-8"><div className="relative flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-medium text-white/75">Your coupon wallet</p><p className="mt-2 text-2xl font-bold">You have <span className="text-amber-200">6 available coupons</span></p><p className="mt-2 max-w-xl text-sm leading-6 text-white/75">Select a coupon and apply it at checkout to enjoy extra savings.</p></div><span className="flex size-20 shrink-0 items-center justify-center rounded-3xl bg-white/10 backdrop-blur"><TicketPercent className="size-9" /></span></div></div>
      ...
      <div className="mt-5 grid gap-5 md:grid-cols-3">{offers.map((offer) => { const Icon = offer.icon; return <article key={offer.title} className="group overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_10px_35px_rgba(15,23,42,0.05)] transition hover:-translate-y-1 hover:shadow-[0_18px_40px_rgba(15,23,42,0.1)]"><div className="relative h-36 overflow-hidden"><img src={offer.image} alt="" className="size-full object-cover transition duration-500 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-slate-950/45 to-transparent" /><span className={`absolute left-4 top-4 flex size-10 items-center justify-center rounded-xl ${offer.tone}`}><Icon className="size-5" /></span></div><div className="p-5"><h3 className="font-bold text-slate-950">{offer.title}</h3><p className="mt-2 text-xs leading-5 text-slate-500">{offer.description}</p><Link to={offer.href} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-auth-primary px-4 py-2.5 text-xs font-bold text-white hover:bg-auth-primary-hover">View Offers<ArrowRight className="size-3.5" /></Link></div></article> })}</div>
