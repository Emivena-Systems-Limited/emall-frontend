...
function FollowedStoreCard({ store, onUnfollow, viewMode }) {
  ...
  return (
    <article className={`group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_8px_28px_rgba(15,23,42,0.04)] transition hover:-translate-y-0.5 hover:border-auth-primary/20 hover:shadow-[0_16px_38px_rgba(15,23,42,0.08)] ${listView ? 'sm:grid sm:grid-cols-[7.5rem_minmax(0,1fr)] sm:items-center' : ''}`}>
      <Link to={`/stores/${store.id}`} className={listView ? 'block p-3 pr-0' : 'block'}>
        <div className={`relative overflow-hidden bg-gradient-to-br from-red-50 via-white to-amber-50 ${listView ? 'h-24 rounded-xl border border-slate-100' : 'aspect-[16/9]'}`}>
      ...
      <div className={`p-4 sm:p-5 ${listView ? 'flex min-w-0 flex-col justify-between sm:py-3 sm:pl-3 sm:pr-4' : ''}`}>
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0"><Link to={`/stores/${store.id}`} className="block truncate text-base font-extrabold text-slate-950 hover:text-auth-primary">{store.name}</Link><p className="mt-1 flex items-center gap-1.5 text-xs text-slate-500"><MapPin className="size-3.5" />{store.city || 'Location not provided'}{store.region ? `, ${store.region}` : ''}</p></div>
          <Link to={`/stores/${store.id}`} aria-label={`Visit ${store.name}`} className="flex size-9 shrink-0 items-center justify-center rounded-full border border-slate-200 text-slate-500 transition hover:border-auth-primary hover:bg-auth-primary hover:text-white"><ArrowRight className="size-4" /></Link>
        </div>
