...
export const progressLog = [
  ...
  },
  {
    ...
    items: [
      ...
      'Removed the extra divider above the Visit Store row while keeping the divider below it',
      'Changed the featured product image to object-contain at all breakpoints and hid duplicate color variant chips under the swatches',
      'Removed dummy About This Item text from the Key Details card while keeping the Key Details section',
      'Limited product details fallback review cards to three visible reviews',
  ...
  },
  {
    ...
    items: [
      ...
      'Wired the user address endpoints with checkout selection and support for saving multiple delivery addresses; default addresses are selected when the API returns is_default. Saved-address creation uses the backend schema: type, first_name, last_name, phone_number, region, city_or_town, address_line_1, and country.',
      'Successful address saves now appear immediately as selectable delivery-address cards; customers can add multiple addresses, switch the active checkout address, and keep backend-fetched addresses reconciled without duplicates.',
      'Corrected saved-address response normalization: delivery addresses are returned in data.shipping and now persist visibly after checkout-page refreshes.',
      ...
      'Billing addresses can now be created from checkout with type billing, then selected independently from the saved billing list.',
      'Saved delivery and billing addresses can now be edited directly on checkout through PUT /user/address/update/{address_id}; the existing form is prefilled and refreshed after a successful update.',
      'Aligned checkout address requests with the revised backend routes: GET /user/address/get, POST /user/address/create, PUT /user/address/update/{address_id}, and the documented DELETE /user/address/delete/{address_id}.',
      'Added guarded delivery and billing address deletion from checkout cards, including pending feedback, refreshed selections, and responsive mobile card actions.',
      'Built /account/addresses as a responsive delivery and billing address manager that shares the checkout address service and React Query cache for create, edit, default selection, and guarded deletion.',
      'Reused checkout’s searchable Ghana region and dependent city/town selectors on the account address form, including custom-town support and backend label conversion.',
      'Prevented account-address update validation errors by normalizing optional address values and omitting blank landmark and delivery_note fields instead of sending empty strings that the backend converts to null.',
      'Matched profile editing to registration with searchable dependent Ghana Region, City/Town, and District selectors, including prefilled legacy values and custom-location fallbacks.',
      'Resolved relative avatar paths from the profile API against the backend storage origin and added an initials fallback when an avatar image cannot load.',
      'Added an immediate object-URL avatar preview after upload so the account thumbnail updates even while the backend storage URL is inaccessible; failed uploads clear the preview safely.',
