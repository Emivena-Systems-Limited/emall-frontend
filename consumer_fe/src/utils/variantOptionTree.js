...
/**
 * Group all variants into independent "families."
 * Linked trees (primary_variant_id) stay together, then families that share the
 * same primary attribute — Color, Size, etc. — are merged so shoppers see one picker.
 */
...
export function resolveInitialFamilyState(families = [], preferredPrimaryValue = '') {
  ...
  families.forEach((family) => {
    const matched = preferredPrimaryValue
      ? family.primaryValues.find((value) => isSameVariantOption(value, preferredPrimaryValue))
      : null
    const primaryVal = matched ?? family.primaryValues[0] ?? ''
...
export function findPurchasableLeaf(variants, selections = {}, family = null) {
  ...
  const requiredKeys = family
    ? [family.primaryKey, ...(family.secondaryGroups ?? []).map((group) => group.key)].filter(Boolean)
    : entries.map(([key]) => key)

  const complete = matches.filter((variant) => (
...
export function buildVariantOptionGroups(variants = []) {
  const store = {}

  const resolveGroupKey = (attributeKey) => {
    const normalized = String(attributeKey ?? '').trim()
    if (!normalized) return ''
    const existing = Object.keys(store).find((key) => attributeKeysMatch(key, normalized))
    return existing || normalized
  }

  const addOption = (attributeKey, valueText, imageUrl) => {
    if (!attributeKey || !valueText) return
    const groupKey = resolveGroupKey(attributeKey)
    if (!store[groupKey]) {
      store[groupKey] = { values: [], images: {}, seen: new Set() }
    }
    const group = store[groupKey]
    const seenKey = String(valueText).trim().toLowerCase()
    if (!group.seen.has(seenKey)) {
      group.seen.add(seenKey)
      group.values.push(valueText)
    }
    if (!imageUrl) return
    const existingImageKey = Object.keys(group.images).find((key) => attributeKeysMatch(key, valueText))
    if (!existingImageKey) group.images[valueText] = imageUrl
  }

  variants.forEach((variant) => {
    const entries = normalizeVariantAttributeEntries(variant.attributes)
    const image = collectVariantImageUrls(variant)[0] || ''

    if (entries.length > 0) {
      entries.forEach((entry) => {
        addOption(entry.name, entry.value, entry.is_primary ? image : '')
      })
      return
    }
    ...
    if (primary?.name && primary?.value) {
      addOption(primary.name, primary.value, image)
    }
  })

  return Object.entries(store).map(([key, group]) => ({
    key,
    label: formatGroupLabel(key),
    values: group.values,
    images: group.images,
...
export function resolveInitialOptionSelections(variants = [], groups = [], preferredPrimaryValue = '') {
  const preferredLeaf = preferredPrimaryValue
    ? variants.find((variant) => {
      const primary = resolvePrimaryAttribute(variant)
      return isSameVariantOption(primary?.value, preferredPrimaryValue)
    }) ?? null
    : null
  const leaf = preferredLeaf ?? variants[0] ?? null
